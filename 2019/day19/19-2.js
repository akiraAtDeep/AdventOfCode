...
for (var y = 1165; y < 100000; y++) {
    console.log(y);
    var find = false;
    var topX = 0;
    var topY = 0;

    var actual = findFirstLast(y);
    var forward = findFirstLast(y + 100);

    if (forward.first + 100 === actual.last) {
        //trovato
        find = true;
        topX = forward.first;
        topY = y;
        ...
        var back = findFirstLast(y - 100);
        if (actual.first + 100 === back.last) {
            //trovato
            find = true;
            topX = actual.first;
            topY = y - 100;
    ...
    if (find) {
        ...
        console.log(topX * 10000 + topY);
    }
}

//console.log(findFirstLast(28));




function findFirstLast(y) {
    var findFirst = false;
    var first = 0;
    var last = 0;
    for (var x = 0; x < 100000; x++) {
        input = {'x': x, 'y': y};
        runIntcode();
        if (!findFirst && input.value === 1) {
            findFirst = true;
            first = x;
        } else if (findFirst && input.value === 0) {
            last = x - 1;
            x = 100000;
        }
    }
