...
cross.forEach(function(element){
	var dist = point['a'].indexOf(element) + point['b'].indexOf(element) + 2;
    minDist = minDist === null ? dist : (minDist < dist ? minDist : dist);
   });

console.log(minDist);

function draw(wire,x,y,thisWire,otherWire) {
	wire.forEach(function(element){
		var step = parseInt(element.substr(1, element.length-1));
		for (var i=0; i<step; i++){
			...
			if (point[otherWire].indexOf(x+'_'+y) !== -1) {
				cross.push(x+'_'+y);
			}
			point[thisWire].push(x+'_'+y);
		}
	});
};